...
type ParkingManager struct {
	...
	TaxiFleet    []TaxiFleetVehicle
	TaxiRequests []TaxiRequest
}

type TaxiFleetVehicle struct {
	X, Z             float32
	Speed            float32
	Moving           bool
...
func (pm *ParkingManager) allocBusDepot() int32 {
	...
	pm.BusDepots[idx].Lifecycle = core.LifecycleAllocated
	pm.DepotCount++
	return int32(idx)
}
...
func (pm *ParkingManager) AllocLot() int32 {
	...
	pm.Lots[idx].Lifecycle = core.LifecycleInitializing
	pm.LotCount++
	return idx
}
...
func (pm *ParkingManager) freeBusDepot(slot int32) {
	...
	pm.BusDepots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) NearestBusDepot(x, z float32, maxDist float32) (int32, float32) {
	...
	bestIdx := int32(-1)
	for i := 0; i < BusDepotPoolSize; i++ {
		if pm.BusDepots[i].Lifecycle != core.LifecycleAllocated {
			continue
		}
		d := &pm.BusDepots[i]
		dx := d.X - x
...
func (pm *ParkingManager) allocTramDepot() int32 {
	...
	pm.TramDepots[idx].Lifecycle = core.LifecycleAllocated
	pm.TramDepotCount++
	return int32(idx)
}
...
func (pm *ParkingManager) freeTramDepot(slot int32) {
	...
	pm.TramDepots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) NearestTramDepot(x, z float32, maxDist float32) (int32, float32) {
	...
	bestIdx := int32(-1)
	for i := 0; i < TramDepotPoolSize; i++ {
		if pm.TramDepots[i].Lifecycle != core.LifecycleAllocated {
			continue
		}
		d := &pm.TramDepots[i]
		dx := d.X - x
...
func (pm *ParkingManager) allocMetroDepot() int32 {
	...
	pm.MetroDepots[idx].Lifecycle = core.LifecycleAllocated
	pm.MetroDepotCount++
	return int32(idx)
}
...
func (pm *ParkingManager) freeMetroDepot(slot int32) {
	...
	pm.MetroDepots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) NearestMetroDepot(x, z float32, maxDist float32) (int32, float32) {
	...
	bestIdx := int32(-1)
	for i := 0; i < MetroDepotPoolSize; i++ {
		if pm.MetroDepots[i].Lifecycle != core.LifecycleAllocated {
			continue
		}
		d := &pm.MetroDepots[i]
		dx := d.X - x
...
func (pm *ParkingManager) allocFerryDepot() int32 {
	...
	pm.FerryDepots[idx].Lifecycle = core.LifecycleAllocated
	pm.FerryDepotCount++
	return int32(idx)
}
...
func (pm *ParkingManager) freeFerryDepot(slot int32) {
	...
	pm.FerryDepots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) NearestFerryDepot(x, z float32, maxDist float32) (int32, float32) {
	...
	bestIdx := int32(-1)
	for i := 0; i < FerryDepotPoolSize; i++ {
		if pm.FerryDepots[i].Lifecycle != core.LifecycleAllocated {
			continue
		}
		d := &pm.FerryDepots[i]
		dx := d.X - x
...
func (pm *ParkingManager) allocMonorailDepot() int32 {
	...
	pm.MonorailDepots[idx].Lifecycle = core.LifecycleAllocated
...
func (pm *ParkingManager) freeMonorailDepot(slot int32) {
	...
	pm.MonorailDepots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) NearestMonorailDepot(x, z float32, maxDist float32) (int32, float32) {
	...
	bestIdx := int32(-1)
	for i := 0; i < MonorailDepotPoolSize; i++ {
		if pm.MonorailDepots[i].Lifecycle != core.LifecycleAllocated {
			continue
		}
...
func (pm *ParkingManager) allocCableCarDepot() int32 {
	...
	pm.CableCarDepots[idx].Lifecycle = core.LifecycleAllocated
...
func (pm *ParkingManager) freeCableCarDepot(slot int32) {
	...
	pm.CableCarDepots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) NearestCableCarDepot(x, z float32, maxDist float32) (int32, float32) {
	...
	bestIdx := int32(-1)
	for i := 0; i < CableCarDepotPoolSize; i++ {
		if pm.CableCarDepots[i].Lifecycle != core.LifecycleAllocated {
			continue
		}
...
func (pm *ParkingManager) allocTaxiDepot() int32 {
	...
	pm.TaxiDepots[idx].Lifecycle = core.LifecycleAllocated
	pm.TaxiDepotCount++
	return int32(idx)
}
...
func (pm *ParkingManager) freeTaxiDepot(slot int32) {
	...
	pm.TaxiDepots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) NearestTaxiDepot(x, z float32, maxDist float32) (int32, float32) {
	...
	bestIdx := int32(-1)
	for i := 0; i < TaxiDepotPoolSize; i++ {
		if pm.TaxiDepots[i].Lifecycle != core.LifecycleAllocated {
			continue
		}
		d := &pm.TaxiDepots[i]
		dx := d.X - x
...
func (pm *ParkingManager) allocAirportDepot() int32 {
	...
	pm.AirportDepots[idx].Lifecycle = core.LifecycleAllocated
...
func (pm *ParkingManager) freeAirportDepot(slot int32) {
	...
	pm.AirportDepots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) NearestAirportDepot(x, z float32, maxDist float32) (int32, float32) {
	...
	bestIdx := int32(-1)
	for i := 0; i < AirportDepotPoolSize; i++ {
		if pm.AirportDepots[i].Lifecycle != core.LifecycleAllocated {
			continue
		}
...
func (pm *ParkingManager) allocPortDepot() int32 {
	...
	pm.PortDepots[idx].Lifecycle = core.LifecycleAllocated
	pm.PortDepotCount++
	return int32(idx)
}
...
func (pm *ParkingManager) freePortDepot(slot int32) {
	...
	pm.PortDepots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) NearestPortDepot(x, z float32, maxDist float32) (int32, float32) {
	...
	bestIdx := int32(-1)
	for i := 0; i < PortDepotPoolSize; i++ {
		if pm.PortDepots[i].Lifecycle != core.LifecycleAllocated {
			continue
		}
		d := &pm.PortDepots[i]
		dx := d.X - x
...
func (pm *ParkingManager) freeLot(slot int32) {
	...
	pm.Lots[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (pm *ParkingManager) PlaceParkingLot(x, z, w, d float32, isGarage bool) bool {
	...
	lot := &pm.Lots[slot]
	lot.Entity = core.NewEntity(pm.NextID, x, 0, z, core.OwnerBuilding)
...
func (pm *ParkingManager) ForEachLot(fn func(*ParkingLot, int32)) {
	for i := 0; i < ParkingLotPoolSize; i++ {
		if pm.Lots[i].Lifecycle == core.LifecycleActive {
			fn(&pm.Lots[i], int32(i))
		}
	}
}

func (pm *ParkingManager) Draw(h *terrain.Heightmap) {
	for _, sp := range pm.Spots {
		if sp.Occupied {
			continue
		}
	...
	for i := 0; i < BusDepotPoolSize; i++ {
		d := &pm.BusDepots[i]
		if d.Lifecycle != core.LifecycleAllocated {
			continue
		}
...
func (pm *ParkingManager) Update(tm TransportCoordinator) {
	pm.Timer++
	...
	if pm.Timer%120 == 0 && pm.TaxiDepotCount > 0 && len(pm.TaxiFleet) < int(pm.TaxiDepotCount)*3 {
		...
		for di := 0; di < TaxiDepotPoolSize; di++ {
			if pm.TaxiDepots[di].Lifecycle == core.LifecycleAllocated {
				pm.TaxiFleet = append(pm.TaxiFleet, TaxiFleetVehicle{
...
func depotStillAlive(pm *ParkingManager, dt uint8, slot int32) bool {
	...
	switch dt {
	case DepotBus:
		return slot < BusDepotPoolSize && pm.BusDepots[slot].Lifecycle == core.LifecycleAllocated
	case DepotTram:
		return slot < TramDepotPoolSize && pm.TramDepots[slot].Lifecycle == core.LifecycleAllocated
	case DepotMetro:
		return slot < MetroDepotPoolSize && pm.MetroDepots[slot].Lifecycle == core.LifecycleAllocated
	case DepotFerry:
		return slot < FerryDepotPoolSize && pm.FerryDepots[slot].Lifecycle == core.LifecycleAllocated
	case DepotMonorail:
		return slot < MonorailDepotPoolSize && pm.MonorailDepots[slot].Lifecycle == core.LifecycleAllocated
	case DepotCableCar:
		return slot < CableCarDepotPoolSize && pm.CableCarDepots[slot].Lifecycle == core.LifecycleAllocated
	case DepotTaxi:
		return slot < TaxiDepotPoolSize && pm.TaxiDepots[slot].Lifecycle == core.LifecycleAllocated
	case DepotAirport:
		return slot < AirportDepotPoolSize && pm.AirportDepots[slot].Lifecycle == core.LifecycleAllocated
	case DepotPort:
		return slot < PortDepotPoolSize && pm.PortDepots[slot].Lifecycle == core.LifecycleAllocated
	}
	return false
}

func (pm *ParkingManager) depotMaintenance(tm TransportCoordinator) {
	if tm == nil {
		return
	}

	tm.ForEachDepotVehicle(func(v *DepotVehicleRef) {
		...
		budgetFactor := tm.LineBudget(v.LineID) / 100.0
		if budgetFactor < 0.5 {
			budgetFactor = 0.5
		}
...
func (pm *ParkingManager) Unload() {
	for i := 0; i < ParkingLotPoolSize; i++ {
		if pm.Lots[i].Lifecycle == core.LifecycleActive {
			pm.Lots[i].Lifecycle = core.LifecycleReturnedToPool
		}
	}
	pm.LotCount = 0
	pm.Spots = nil
...
// DepotVehicleRef exposes mutable depot-linked vehicle fields for maintenance.
type DepotVehicleRef struct {
	HomeDepotSlot    int32
	HomeDepotType    uint8
	LineID           uint32
	Maintenance      *float32
...
// TransportCoordinator supplies transport data parking maintenance needs without importing transport.
type TransportCoordinator interface {
	ForEachDepotVehicle(fn func(v *DepotVehicleRef))
	LineBudget(lineID uint32) float32
}
