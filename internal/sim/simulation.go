...
func NewSimulationManager(seed int64) *SimulationManager {
	...
	sm.Zones = zoning.NewZoneManager(512, 512, sm.Roads, sm.Buildability)
	sm.Zones.Init()
...
func (sm *SimulationManager) collectTax(dt float64) {
	...
	if sm.TaxTimer > 60 {
		...
		if sm.Transport != nil {
			...
			income := sm.Transport.TotalIncome() * 0.1
			sm.Money += income
			sm.EventBus.Emit(string(core.EventTaxCollected), income)
		}
		sm.TaxTimer = 0
	}
}
