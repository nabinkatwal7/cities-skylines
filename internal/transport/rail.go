...
func NewRailManager() *RailManager {
	...
	for i := 0; i < RailTrackPoolSize; i++ {
		rm.Pool[i].Lifecycle = core.LifecycleUnallocated
...
func (rm *RailManager) allocTrack() int32 {
	...
	rm.Pool[idx].Lifecycle = core.LifecycleAllocated
	rm.Count++
	return int32(idx)
}
...
func (rm *RailManager) freeTrack(slot int32) {
	...
	rm.Pool[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (rm *RailManager) propagateBlock(slot int32, blockID int32) {
	...
	for j := 0; j < RailTrackPoolSize; j++ {
		...
		other := &rm.Pool[j]
		if other.Lifecycle != core.LifecycleAllocated || other.BlockID >= 0 {
			continue
		}
...
func (rm *RailManager) ForEachTrack(fn func(*RailTrack, int32)) {
	for i := 0; i < RailTrackPoolSize; i++ {
		if rm.Pool[i].Lifecycle == core.LifecycleAllocated {
...
func (rm *RailManager) FindTrackPath(startX, startZ, endX, endZ float32) []int32 {
	...
	for {
		...
		for j := 0; j < RailTrackPoolSize; j++ {
			if rm.Pool[j].Lifecycle != core.LifecycleAllocated || j == best {
				continue
			}
			other := &rm.Pool[j]
...
func (rm *RailManager) InitOutsideConnections(cs *terrain.ConnectionSystem) {
	for _, conn := range cs.GetByType(terrain.ConnRail) {
		inwardX := conn.WorldX
		inwardZ := conn.WorldZ
		if conn.WorldX < -terrain.WorldSize/2+50 {
			inwardX = conn.WorldX + 80
		} else if conn.WorldX > terrain.WorldSize/2-50 {
			inwardX = conn.WorldX - 80
		}
		if conn.WorldZ < -terrain.WorldSize/2+50 {
			inwardZ = conn.WorldZ + 80
		} else if conn.WorldZ > terrain.WorldSize/2-50 {
			inwardZ = conn.WorldZ - 80
		}
...
func (rm *RailManager) Draw(h *terrain.Heightmap) {
	for i := 0; i < RailTrackPoolSize; i++ {
		t := &rm.Pool[i]
		if t.Lifecycle != core.LifecycleAllocated {
			continue
...
func (cm *CargoManager) Draw(h *terrain.Heightmap) {
