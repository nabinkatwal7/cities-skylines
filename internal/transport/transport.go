...
const stopCellSize = terrain.WorldSize / stopGridSize
...
func stopGridCell(x, z float32) (int, int) {
	cx := int((x + terrain.WorldSize/2) / stopCellSize)
	cz := int((z + terrain.WorldSize/2) / stopCellSize)
	if cx < 0 {
		cx = 0
	}
	if cx >= stopGridSize {
...
func NewTrackManager() *TrackManager {
	...
	for i := 0; i < MetroTrackPoolSize; i++ {
		tm.Pool[i].Lifecycle = core.LifecycleUnallocated
...
func (trk *TrackManager) allocTrack() int32 {
	...
	trk.Pool[idx].Lifecycle = core.LifecycleAllocated
	trk.Count++
	return int32(idx)
}
...
func (trk *TrackManager) freeTrack(slot int32) {
	...
	trk.Pool[slot].Lifecycle = core.LifecycleReturnedToPool
...
func (trk *TrackManager) ForEachTrack(fn func(*MetroTrack, int32)) {
	for i := 0; i < MetroTrackPoolSize; i++ {
		if trk.Pool[i].Lifecycle == core.LifecycleAllocated {
...
func (trk *TrackManager) FindTrackPath(startX, startZ, endX, endZ float32) []int32 {
	...
	for i := 0; i < MetroTrackPoolSize; i++ {
		t := &trk.Pool[i]
		if t.Lifecycle != core.LifecycleAllocated {
			continue
		}
		dx1 := t.StartX - startX
		dz1 := t.StartZ - startZ
	...
	for {
		...
		for j := 0; j < MetroTrackPoolSize; j++ {
			if trk.Pool[j].Lifecycle != core.LifecycleAllocated || j == best {
				continue
			}
			other := &trk.Pool[j]
			shared := false
...
func (tm *TransportManager) findNearestDepot(tt TransportType, x, z, maxDist float32) (uint8, int32, float32, float32) {
	...
	case TransBus:
		...
		if slot >= 0 { return road.DepotBus, slot, tm.Parking.BusDepots[slot].X, tm.Parking.BusDepots[slot].Z }
	case TransTram:
		slot, _ := tm.Parking.NearestTramDepot(x, z, maxDist)
		if slot >= 0 { return road.DepotTram, slot, tm.Parking.TramDepots[slot].X, tm.Parking.TramDepots[slot].Z }
	case TransMetro:
		slot, _ := tm.Parking.NearestMetroDepot(x, z, maxDist)
		if slot >= 0 { return road.DepotMetro, slot, tm.Parking.MetroDepots[slot].X, tm.Parking.MetroDepots[slot].Z }
	case TransFerry:
		slot, _ := tm.Parking.NearestFerryDepot(x, z, maxDist)
		if slot >= 0 { return road.DepotFerry, slot, tm.Parking.FerryDepots[slot].X, tm.Parking.FerryDepots[slot].Z }
	case TransMonorail:
		slot, _ := tm.Parking.NearestMonorailDepot(x, z, maxDist)
		if slot >= 0 { return road.DepotMonorail, slot, tm.Parking.MonorailDepots[slot].X, tm.Parking.MonorailDepots[slot].Z }
	case TransCableCar:
		slot, _ := tm.Parking.NearestCableCarDepot(x, z, maxDist)
		if slot >= 0 { return road.DepotCableCar, slot, tm.Parking.CableCarDepots[slot].X, tm.Parking.CableCarDepots[slot].Z }
	case TransTaxi:
		slot, _ := tm.Parking.NearestTaxiDepot(x, z, maxDist)
		if slot >= 0 { return road.DepotTaxi, slot, tm.Parking.TaxiDepots[slot].X, tm.Parking.TaxiDepots[slot].Z }
	case TransAir:
		slot, _ := tm.Parking.NearestAirportDepot(x, z, maxDist)
		if slot >= 0 { return road.DepotAirport, slot, tm.Parking.AirportDepots[slot].X, tm.Parking.AirportDepots[slot].Z }
	case TransShip:
		slot, _ := tm.Parking.NearestPortDepot(x, z, maxDist)
		if slot >= 0 { return road.DepotPort, slot, tm.Parking.PortDepots[slot].X, tm.Parking.PortDepots[slot].Z }
	}
	return 0, -1, x, z
}
...
func (tm *TransportManager) SpawnVehicle(lineIdx int) {
	...
	stop0 := tm.StopByID(line.Stops[0])
	if stop0 == nil {
		return
	}
...
func (tm *TransportManager) InitExternalConnections(cs *terrain.ConnectionSystem) {
	if cs == nil {
		return
	}
	for _, c := range cs.GetByType(terrain.ConnAir) {
	...
	for _, c := range cs.GetByType(terrain.ConnShip) {
	...
	for _, c := range cs.GetByType(terrain.ConnRail) {
...
func (tm *TransportManager) Update(rm *road.RoadManager, h *terrain.Heightmap) {
	for i := 0; i < TransportVehiclePoolSize; i++ {
...
func (tm *TransportManager) updatePoolVehicles(rm *road.RoadManager, h *terrain.Heightmap) {
	for i := 0; i < TransportVehiclePoolSize; i++ {
		v := &tm.Pool[i]
...
func (tm *TransportManager) updateSliceVehicles(h *terrain.Heightmap) {
...
func (tm *TransportManager) moveVehicle(v *TransportVehicle, rm *road.RoadManager, h *terrain.Heightmap) {
	lineIdx := -1
	for li, line := range tm.Lines {
		if line.ID == v.LineID {
			lineIdx = li
...
func (tm *TransportManager) moveDirect(v *TransportVehicle, rm *road.RoadManager, h *terrain.Heightmap, nextStop, currentStop *TransportStop, line *TransportLine, nextIdx int) {
...
func (tm *TransportManager) moveBusOnRoad(v *TransportVehicle, rm *road.RoadManager, h *terrain.Heightmap, nextStop, currentStop *TransportStop, line *TransportLine, nextIdx int) {
	...
	if len(v.Path) == 0 {
		...
		v.Path = rm.FindPath(startNode, endNode, int(road.VehicleBus))
		v.PathIdx = 0
		if len(v.Path) == 0 {
	...
	for v.PathIdx < len(v.Path)-1 {
		...
		if targetDist < 2 {
			...
			// Traffic light check
			if node.TrafficLight != road.TrafficLightNone && int(nodeIdx) < len(rm.Nodes) {
				if node.TrafficLight == road.TrafficLightRed || node.TrafficLight == road.TrafficLightYellow {
					v.Delay++
					return
				}
			}
...
func (tm *TransportManager) moveTramOnTracks(v *TransportVehicle, rm *road.RoadManager, h *terrain.Heightmap, nextStop, currentStop *TransportStop, line *TransportLine, nextIdx int) {
	if len(v.Path) == 0 {
		startNode, startOK := rm.NearestNode(v.X, v.Z)
		endNode, endOK := rm.NearestNode(nextStop.X, nextStop.Z)
		if !startOK || !endOK {
		...
		v.Path = rm.FindPath(startNode, endNode, int(road.VehicleTram))
		v.PathIdx = 0
		if len(v.Path) == 0 {
			v.Path = append(v.Path, startNode, endNode)
		}
...
func (tm *TransportManager) moveMetroOnTracks(v *TransportVehicle, h *terrain.Heightmap, nextStop, currentStop *TransportStop, line *TransportLine, nextIdx int) {
	if len(v.Path) == 0 {
...
func (tm *TransportManager) moveTrainOnRails(v *TransportVehicle, h *terrain.Heightmap, nextStop, currentStop *TransportStop, line *TransportLine, nextIdx int) {
	if len(v.Path) == 0 {
...
func (tm *TransportManager) applyCongestion(rm *road.RoadManager, fromNode, toNode int, baseSpeed float32) float32 {
	for _, sid := range rm.Nodes[fromNode].Connected {
		...
		if int(seg.NodeA) == toNode || int(seg.NodeB) == toNode {
			...
			switch road.HierarchyForRoad(seg.RoadType) {
			case road.HierarchyLocal:
...
func (tm *TransportManager) drawVehicle(v *TransportVehicle, h *terrain.Heightmap) {
...
// ForEachDepotVehicle implements road.TransportCoordinator.
func (tm *TransportManager) ForEachDepotVehicle(fn func(v *road.DepotVehicleRef)) {
	tm.forEachVehicle(func(v *TransportVehicle, _ int32) {
		fn(&road.DepotVehicleRef{
...
// LineBudget implements road.TransportCoordinator.
func (tm *TransportManager) LineBudget(lineID uint32) float32 {
	for li := range tm.Lines {
		if tm.Lines[li].ID == lineID {
			return tm.Lines[li].Budget
		}
	}
	return 100
}
